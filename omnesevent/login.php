...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($utilisateur && password_verify($mot_de_passe, $utilisateur['mot_de_passe_hash'])) {
        if ($utilisateur['role'] === 'organisateur' && (int)$utilisateur['est_approuve'] === 0) {
            ...
            if ($utilisateur['role'] === 'admin') header('Location: /omnesevent/dashboard/admin.php');
            elseif ($utilisateur['role'] === 'organisateur') header('Location: /omnesevent/dashboard/organizer.php');
            else header('Location: /omnesevent/index.php');
            exit;
        }
    } else {
...
<section class="auth-page">
    <form class="auth-card validate-form" method="post">
        ...
        <p class="muted">Pas encore de compte ? <a href="/omnesevent/register.php">Inscription</a></p>
    </form>
</section>
